    ...
    <Stack gap="md">
      ...
      {(joinRequest.jobTitle ||
        ...
        <Paper withBorder p="md" component={Stack}>
          ...
          {(joinRequest.expectedSalaryMin || joinRequest.expectedSalaryMax) && (
            <InfoItem
      ...
      {joinRequest.createdAt && (
        <Paper withBorder p="md" component={Stack}>
          <Title order={3}>{t('joinRequests.metadata')}</Title>
          <InfoItem
            icon={IconCalendar}
            label={t('joinRequests.createdAt')}
            value={
              <Text dir="ltr">
                {formatDateTime(joinRequest.createdAt as string)}
              </Text>
            }
          />
        </Paper>
